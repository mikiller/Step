...
public class ExtractVideoInfoUtil {
    ...
    public ExtractVideoInfoUtil(String path, Handler handler) {
    ...
    public String getMimeType(){
        return mMetadataRetriever.extractMetadata(mMetadataRetriever.METADATA_KEY_MIMETYPE);
    }
...
//        Bitmap bitmap = mMetadataRetriever.getFrameAtTime(timeMs * 1000, MediaMetadataRetriever.OPTION_CLOSEST);
     ...
     * 获取视频某一帧,不一定是关键帧
     *
     * @param startTimeMs 起始毫秒
     * @param maxCount 最大采样数
     */
    public void extractFrame(long startTimeMs, double newWidth, double newHeight, int maxCount) {
        //第一个参数是传入时间，只能是us(微秒)
        //OPTION_CLOSEST ,在给定的时间，检索最近一个帧,这个帧不一定是关键帧。
        //OPTION_CLOSEST_SYNC   在给定的时间，检索最近一个同步与数据源相关联的的帧（关键帧）
        //OPTION_NEXT_SYNC 在给定时间之后检索一个同步与数据源相关联的关键帧。
        //OPTION_PREVIOUS_SYNC  顾名思义，同上
        long interval = (fileLength - startTimeMs) / (maxCount - 1);
        for (long i = startTimeMs; i < maxCount; i ++) {
            if(stop)
                break;
            long time = (startTimeMs + interval * i) * 1000;
            Bitmap tmpBmp = mMetadataRetriever.getFrameAtTime(time, MediaMetadataRetriever.OPTION_CLOSEST_SYNC);
            if (tmpBmp != null && extractHandler != null) {
                Bitmap newBmp = getScaledBitmap(tmpBmp, newWidth, newHeight);
                Message msg = extractHandler.obtainMessage(ExtractFrameWorkThread.MSG_SAVE_SUCCESS);
                msg.obj = newBmp;
                extractHandler.sendMessage(msg);
    ...
    public static Bitmap getScaledBitmap(Bitmap srcBmp, double newWidth, double newHeight){
        float scaleW = (float) (newWidth / srcBmp.getWidth()), scaleH = (float) (newHeight / srcBmp.getHeight());
